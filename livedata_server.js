...
var Metoer = require('./meteor_shim.js');
...
_.extend(Session.prototype, {
  ...
  send: function (msg) {
    ...
    if (self.socket) {
      if (utils.isDebug)
        utils._debug("Sent DDP", stringifyDDP(msg));
  ...
  self.stream_server.register(function (socket) {
    ...
    socket.on('data', function (raw_msg) {
      if (Meteor._printReceivedDDP) {
        utils._debug("Received DDP", raw_msg);
      }
      try {
        try {
        ...
        utils._debug("Internal exception while processing message", msg,
                      e.message, e.stack);
      }
    });
...
_.extend(Server.prototype, {
  ...
  publish: function (name, handler, options) {
    ...
    if (name && name in self.publish_handlers) {
      utils._debug("Ignoring duplicate publish named '" + name + "'");
      return;
    }
...
var wrapInternalException = function (exception, context) {
  ...
  if (!exception.expected) {
    utils._debug("Exception " + context, exception.stack);
    if (exception.sanitizedError) {
      utils._debug("Sanitized and reported to the client as:", exception.sanitizedError.message);
      utils._debug();
    }
  }
  ...
  if (exception.sanitizedError) {
    ...
    utils._debug("Exception " + context + " provides a sanitizedError that " +
